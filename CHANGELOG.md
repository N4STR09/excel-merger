...
⚠️ **`mvnw verify` no ha sido ejecutado** en este entorno (sin Maven Central). Validación basada en:

- Revisión línea a línea del código nuevo (`ResponsablePivotBuilder`, `ResponsablesSheetBuilder` reescrito, `PoiUtils.mixedNumericLexicographicSort`, `ConfigValidator.validateResponsablesTables`).
- Tests añadidos diseñados con FormulaEvaluator para verificar matemática real, no solo presencia de strings.
- `ResponsablesSheetBuilderTest` y resto de tests v2.3.0 NO se han modificado en sus aserciones (solo el helper `minimalConfig` añade un opt-out).

**Antes de publicar, correr `./mvnw verify` localmente** y confirmar tests verdes y cobertura ≥70%.
